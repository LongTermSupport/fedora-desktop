...
**The installed key is found by ID, never by package name or description.** The play
acts on a `refresh` with `rpm -e`, so the set this module hands it has to be incapable
of naming a key nobody asked about. Every installed `gpg-pubkey` package is opened and
its own armour parsed, and only those whose PRIMARY equals the published one can reach
the erase set.

The package name is deliberately not a filter. rpm's `gpg-pubkey` version is an rpm
packaging detail — the primary's short id for years, the full fingerprint under rpm 6 —
so selecting on it decided identity from a string rpm is free to change. It changed:
on Fedora 44 a key that was installed and verifying packages was reported absent, the
play imported nothing because `rpm_key` could see it perfectly well, and the
post-condition below failed on that host on every run.
...
@dataclass(frozen=True)
class InstalledKey:
    """One installed `gpg-pubkey` package, and the key ids its armour actually holds."""
...
def installed_envelopes(*, run: Runner = subprocess.run) -> list[str]:
    """Every installed `gpg-pubkey` package, under whatever name rpm gave it.

    The listing is deliberately NOT filtered by name. rpm's `gpg-pubkey` version is
    the primary's short id under rpm 4 and 5 and the key's full fingerprint under
    rpm 6, so a `gpg-pubkey-<short id>-` prefix match silently stopped selecting
    anything on Fedora 44 — and "nothing matched" is indistinguishable from "the key
    is absent" to everything downstream. Identity is settled by `needs_refresh`
    instead, from the primary parsed out of each package's own armour, which is the
    one thing rpm cannot rename.

    Reading every key costs one `rpm -q` and one `gpg` each, over the tens of keys a
    desktop holds. That is the price of not deciding identity from a filename.

    An empty list means rpm answered and the keyring is empty — a normal state the
    caller handles by importing. A FAILING `rpm` is left to raise: "rpm could not
    tell me" and "there is no such key" are different facts, and reporting the first
    as the second would re-import a key on every run for ever.
    """
    listing = run(
    ...
    # A blank line would become an `rpm -q` argument naming every installed package.
...
def needs_refresh(
    *, installed: Sequence[InstalledKey], published: dict[str, Any]
    ...
    """Decide what to do, given EVERY `gpg-pubkey` package installed on the host.
    ...
    # Identity is the primary id and nothing else. Every other key in the keyring is a
    # stranger: not ours to remove, and not evidence ours is present either. An
    # unreadable key has `primary=None`, which is not a match, so it is spared rather
    # than erased on a guess.
    ...
    if not ours:
        return Verdict(
            ...
            import_missing=True,
            reason=(
                f"no installed key has the published primary {published['primary']}; "
                "it needs importing, not refreshing"
            ),
        )
...
def main(
    ...
    *,
    ...
    # EVERY key in the keyring is read, not a name-selected subset. The verdict's erase
    # set becomes an argument to `rpm --erase`, so a package that was never opened must
    # not be able to reach it — and a package whose NAME does not advertise the key it
    # holds is the state that broke this on Fedora 44.
    installed = []
    for envelope in installed_envelopes(run=run):
