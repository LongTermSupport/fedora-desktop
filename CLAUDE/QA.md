...
| `test-run-bash-gh-scopes.bash`              | GitHub scopes asked for in one pass, from `vars/github-required-scopes.yml`: `run.bash`'s login carries every scope, a short token gets one refresh naming all it lacks, and headless `run.bash` and `gh-account-setup.bash` fail once naming every account and scope (Plan 00139 T1.3)                                                                           |
