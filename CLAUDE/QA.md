...
| Script                   | Checks                                                                                                                                                                                                                                                                       | Files                                                                                              |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| `qa-bash.bash`           | `bash -n` (always) + shellcheck (**required** — exits 2 if absent, Plan 00075). Exits 2 if discovery finds **0 files**. **shellcheck `error`-level findings GATE**; `warning`/`info`/`style` advisory.                                                                       | Repo-owned bash (excludes `roles/vendor`, `.claude/hooks-daemon`, `.claude/ccy`, `.claude/skills`) |
...
- ✅ shellcheck `error`-level findings (`warning`/`info`/`style` advisory). **shellcheck is required** — absent, the stage exits 2 rather than reporting a pass it did not earn (Plan 00075)
