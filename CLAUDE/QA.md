...
| `test-run-bash-single-play.bash`            | the real `run.bash` single-play mode in a throwaway checkout: `--headless <play>` unattended (sudo-only preflight, password via `/dev/fd/N`, stdin closed, the play's exit status) and the shared play lock (exit 75 when held, delegation proven) (Plan 00137 T1.4/T2.1) |
