...
`qa-all.bash` runs **thirty-two** gates. Seven merge their JSON into
`/tmp/qa-results.json`; the other twenty-five run separately (see below). A missing
...
Twenty-five further gates run inside `qa-all.bash` as **hard, non-structural** checks —
...
| `test-ccy-sessions-status.bash`             | `ccy-sessions restore-status` and the blocked reboot seam: every installation state including the two "could not tell" ones, driven through a stub systemctl/loginctl because the broken states are unreachable otherwise (Plan 00123)    |
