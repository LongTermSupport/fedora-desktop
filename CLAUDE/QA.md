...
| `test-bash-history-session.bash`            | with the Ctrl+R search bound, up-arrow offers only the current terminal's commands while each is still appended once to the shared file, even when the prompt hook is replaced; without it (no fzf), the whole file is loaded for stock Ctrl+R. Real interactive shells, rc-file started, run as a normal user (Plan 00138) |
