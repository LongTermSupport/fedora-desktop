...
| `test-bash-history-session.bash`            | up-arrow offers only the current terminal's commands, while each command is still appended to the shared history file Ctrl+R searches, without overwriting it; driven in real interactive shells started with the snippet as their rc file (Plan 00138)                                                           |
