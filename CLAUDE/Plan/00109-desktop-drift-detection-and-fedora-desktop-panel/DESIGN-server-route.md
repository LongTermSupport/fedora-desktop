...
- **The pin check.** On a server `dkms()` raises "command not found", so every
  DKMS-resolved pin became *"could not be checked"* for ever. The owner's decision of
  26-09-24: such a pin is **not applicable** when three things hold together — no DKMS
  state directory, a `dkms` command the OS could not find, and no ledger run of the
  play that installs the pin's software. It then leaves the population the host is
