...
- [x] 🚫 **Task 4.4**: **Closed unrun, by owner decision.** Set a different value via
  `export` in a project's `ccy.env` and confirm the project's value wins. The idiom is
...
- [x] ✅ **Task 4.6**: Run the **`qa-reviewer`** agent as the final step, per
  ...
  the post-mortem had invented its mechanism. Round 7 found the override example in
  `docs/ccy.md` was itself `1M`, which parses to 1 — the documented workaround reproducing
  the documented bug. All findings fixed; reports in `subagent-reports/`.
...
- [x] A project setting the variable with `export` in its tracked
  `.claude/ccy/ccy.env` gets its own value, not `600000`. **Accepted unproven** — the
  shared `${VAR:-default}` idiom, plus a readback that would expose a failure.
- [x] A host export before launch also overrides the CCY default. **Accepted unproven**,
  same idiom.
...
- [x] The `qa-reviewer` agent reports no findings. **Reached at round 7's close**: every
  finding from rounds 6 and 7 is fixed in the tree. Three rounds were needed because the
  first two corrections were each wrong in turn.
