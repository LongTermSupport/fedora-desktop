#!/usr/bin/env bash
#
# Plan 00097 triage — establish grounded facts about the browser engines available
...
usage() {
    cat <<'USAGE'
Plan 00097 triage — browser engine facts for the CCY container.
...
if [ ! -f /run/.containerenv ] && [ ! -f /.dockerenv ]; then
    ...
    if [ -z "$CONTAINER" ]; then
        ...
        if [ "$count" -eq 0 ]; then
            ...
            echo >&2
            echo "  If the image predates Plan 00097 (container 2.23), deploy first:" >&2
...
WORK="$(mktemp -d)"
CACHE="${XDG_CACHE_HOME:-$HOME/.cache}/plan-00097"
mkdir -p "$CACHE"
...
echo "==============================================================================="
echo " Plan 00097 — browser engine triage"
...
echo "## 7. Deployed integration (Plan 00097 Phase 3)"
echo
...
if command -v agent-browser-lite > /dev/null; then
    ...
    echo "  agent-browser-lite is NOT on PATH — this image predates Plan 00097's"
