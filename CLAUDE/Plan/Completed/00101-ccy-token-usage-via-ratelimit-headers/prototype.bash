#!/usr/bin/env bash
#
# Plan 00101 — PROTOTYPE: can a stored ccy setup-token read its own usage from
# the unified rate-limit RESPONSE HEADERS?
#
# HOST-ONLY. The CCY container holds no tokens.
#
# ⚠️  THIS SPENDS REAL QUOTA. Unlike Plan 00100's triage, this is not read-only:
...
#     written or changed.
#
# BACKGROUND: Plan 00100 established that GET /api/oauth/usage answers 403
...
usage() {
    cat <<'EOF'
Plan 00101 prototype — read usage limits from /v1/messages response headers
...
# {n,} intervals — mawk ignores them (Plan 00100 caught this the hard way).
redact() {
...
{
    echo "================================================================================"
    echo "Plan 00101 — usage limits via /v1/messages response headers (PROTOTYPE)"
    echo "================================================================================"
    ...
    echo "         a missing scope, this dies the same way Plan 00100 did. If it"
