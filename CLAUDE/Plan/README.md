...
- [00036-cc-ccy-parity](00036-cc-ccy-parity/) - Bring host `cc` to parity with containerised `ccy` for host-safe terminal UX vars (`CLAUDE_CODE_NO_FLICKER`, `CLAUDE_CODE_DISABLE_MOUSE`) via a new bashrc include with a sentinel-checked `cc` function
...
- [00043-ipu6-webcam-fallout](00043-ipu6-webcam-fallout/) - Incident: `play-ipu6-webcam.yml` (commit e5d0e33) pulled `akmod-intel-ipu6` which dragged in a half-installed kernel 7.0.9 (kernel-core only, no kernel-modules) — killed WiFi+BT on next boot; also one unrelated 7.0.4 freeze. Playbook rewritten to drop akmod (mainline IPU6 is in-tree on F43+) plus userspace-only stack; host recovery blocked on user backup before any further system changes
- [00044-laptop-health-audit](00044-laptop-health-audit/) - Read-only audit (4 parallel sub-agents: kernel/dmesg, systemd/boot, packages/security, power/thermal) of the daily-driver X1 Carbon Gen 10 i7-1260P. System broadly healthy. Prioritised actions: apply Critical security updates, switch `tuned` profile to `balanced-battery`, mask redundant `thermald.service` (DYTC platform), drop `phracek/PyCharm` COPR. Watch items: NM coredump frequency, ELAN touchpad post-resume, `mtk_t7xx` blacklist decision. Blocked on user backup confirmation before any Phase 2/3 changes
