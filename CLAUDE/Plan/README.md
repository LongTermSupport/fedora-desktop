...
- [00042-darktable-ai-features](00042-darktable-ai-features/) - Enable darktable's optional AI features (object masks, denoise, upscale), which neither the Fedora RPM nor the Flatpak builds in — via a local `mock` source build, AppImage as fallback. Awaiting Phase 1 decision gate. (Renumbered from local 00039, which collided with remote 00039.)
...
- [00082-plan-index-hygiene-and-comment-handlers](00082-plan-index-hygiene-and-comment-handlers/) - Enables the three handlers the 3.54.0 daemon upgrade shipped disabled (`comment_changelog`, `comment_size`, `sensitive_content`), each after measuring its existing backlog rather than assuming it, and clears the 39 over-length rows the new `index-row-length` check found in this index.

- [00082-run-bash-github-accounts-none](00082-run-bash-github-accounts-none/) - `run.bash` headless v1 (Plan 00063) hard-required a configured GitHub account; `RUN_BASH_GITHUB_ACCOUNTS=none` failed preflight naming a "planned follow-up, not yet supported." Of the two blockers Plan 00063's Task 1.6 deferral cited, one (the LXC clone's `git@github.com:` dependency) is confirmed fixed on current `main`, and the other (`play-github-cli-multi.yml`'s ungated `gh --version` check) could not be reproduced from static file reading — `play-git-configure-and-tools.yml` already installs `gh` unconditionally, earlier in `playbook-main.yml` — so it is recorded as NOT REPRODUCIBLE rather than asserted fixed, flagged for HOST verification (Phase 5) rather than trusted from the container. Implements the empty path: `GITHUB_ACCOUNTS=none` clones `fedora-desktop` over HTTPS, skips gh install/auth/SSH-key-upload/known-hosts entirely, and still runs full provisioning (identity + vault + main playbook); contradictory config (`RUN_BASH_CONFIG_SOURCE` or `RUN_BASH_RESTORE_PROJECTS=1` combined with `none`, both of which need a GitHub identity) fails fast in preflight. Along the way, verifying this plan's own acceptance coverage surfaced a pre-existing defect in shared, repo-wide `scripts/qa-discovery.bash`/`scripts/qa-js.bash`: file discovery excluded on an *absolute-path* substring match, so any checkout whose ancestor path contains an `untracked` segment (this CCY container's own `/workspace/untracked/repos/fedora-desktop`) excluded every file in the repo — `qa-all.bash` reported "0 bash files" as a hard failure rather than a false pass. Fixed by switching to repo-relative exclusion, unifying with the mechanism the git-based tracked-file discovery already used.
