...
- [00066-ftp-camera-airbnb-wifi-and-hotspot-triage](00066-ftp-camera-airbnb-wifi-and-hotspot-triage/) - Two `ftp-camera` failures on Airbnb WiFi. (1) `--async-copy` never gets past the first frame: the camera re-uploaded the same `DSC06824` ~6 times in 8 minutes. The load-bearing fact is that the log carries **zero `FAIL LOGIN`/`FAIL UPLOAD`** — every transfer vsftpd saw *completed*, so "the WiFi is too weak" does not fit; something makes the camera re-send frames that already landed. Primary (unconfirmed) hypothesis H1: async mode `sudo mv`s the file out of the chroot the instant `OK UPLOAD` logs, so a camera that checks the file back (`SIZE`/`LIST`/`MLSD`) sees it missing and retries — which also explains the no-EXIF ARW (overlapping retries racing one `STOR` path). Cause deliberately **not** asserted: ships `triage.bash` + a new `ftp-camera --debug-ftp` (runtime-only `log_ftp_protocol=YES`, PASS masked) as the decisive H1-vs-H2 probe. (2) `--hotspot` was **confirmed broken from source** — the play only ever *tuned* an NM profile the user was told to create by hand in GNOME Settings, a prohibited manual change, so the profile never existed; the play now creates the WPA2 AP itself (vaulted PSK, pmf disabled for camera compat, ch6/40MHz). Also fixed a `qa-ansible.bash` false positive that flagged the directive-name in a comment. QA green (417 files); HOST triage run pending.
