...
- [00067-rclone-rc-auth-instead-of-no-auth](00067-rclone-rc-auth-instead-of-no-auth/) - `play-rclone.yml` renders `--rc-no-auth` into every mount unit, justified in-repo as "safe because `--rc-addr` binds to localhost only". That premise fails: **loopback is not a user boundary** — every local uid can reach `127.0.0.1:5572`, and rclone's docs state RC access "is equivalent to shell access as the rclone user" (`core/command` runs OS commands, `config/dump` reads back the Drive OAuth token). On this host the `camera` FTP account (uid 1001) is the realistic compromise target, and `joseph` has passwordless sudo, so no-auth converts a service compromise into root + cloud credentials; rootful Docker `--network host` reaches loopback too. The flag also buys almost nothing: a measured endpoint matrix (rclone v1.74.3) shows the tooling's real needs — `vfs/stats`, `core/stats` — are **already auth-free**, leaving only `vfs/refresh` (`rclone-cache-warm`, currently broken) and a cosmetic `core/stats-reset`. Replaces blanket no-auth with a host-generated `0600` secret loaded via systemd `EnvironmentFile=` (keeps it out of `ExecStart`, `ps`, the unit file, and this public repo), and teaches the four RC clients to authenticate. Verified flag facts rather than assuming: server takes `--rc-user`/`--rc-pass`, client takes `--user`/`--pass`, and the `RCLONE_<FLAG>` env mapping was confirmed empirically. Found while diagnosing `ftp-camera --copy`; the separate preflight bug it uncovered (probing auth-gated `core/pid`, an endpoint `--copy` never uses, so it failed closed on a working mount) is **already fixed and deployed** ahead of this plan. Deploy restarts the mount, so it must not run during a `--copy`.
