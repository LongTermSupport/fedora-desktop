...
- [00069-plan-document-size-discipline](00069-plan-document-size-discipline/) - Plan 00068's `PLAN.md` hit 1,894 lines / 137 KB: **50% blockquote corrections, 2% task checkboxes**, never once shrinking across 34 commits. Cause is structural, not careless — "corrections APPEND, never rewrite" is right for `JOURNAL/` and makes `PLAN.md` grow monotonically with review effort. Move corrections to the journal, give `PLAN.md` a size budget, and replace the remembered coherence `grep` (which failed **fifteen** times in 00068, twice while cataloguing itself) with a mechanical check.
