...
- [00060-stderr-hygiene-coding-standard](00060-stderr-hygiene-coding-standard/) - A generated `gh-<alias>()` wrapper echoed its "Switching to <user>..." status line to stdout, polluting `$(gh-<alias> … --json)` captures and breaking `jq` when the account wasn't the box default; the one-word fix is `>&2`. Generalises that into: fix the wrapper, audit the repo (bash executables, generated includes, playbook shell blocks, `scripts/`, `helpers/`) for other "diagnostic-on-stdout-where-stdout-is-captured" bugs, and make "echo diagnostics to stderr when appropriate" a first-class documented coding standard (extending `CLAUDE/InteractiveScripts.md` rule 08).
