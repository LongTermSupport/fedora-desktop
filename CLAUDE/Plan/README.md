...
- [00070-documentation-drift-audit](00070-documentation-drift-audit/) - A read-only audit of `CLAUDE.md`, `CLAUDE/`, `.claude/rules/`, `docs/`, `README.md` and the nested `*/CLAUDE.md` files found **17 confirmed defects and 6 suspected** — **11 of the 17 independently re-verified against the tree** before being recorded, the rest marked as unverified (one, the vault password file, deliberately **not** checked because reading a credential is not something an audit needs). The defect is **drift**, not sloppiness: the code and the newer docs are disciplined; the older docs were never re-read after the changes that invalidated them. **Four docs actively cause harm** — a reader who follows them ends up worse off than if the doc did not exist: `docs/configuration.md:187`, `docs/development.md:129,:293` and `docs/playbooks.md:1016` (the copy-paste template new contributors are told to use) all teach `root_dir: "{{ inventory_dir }}/../../"`, the pattern `CLAUDE/AnsibleStyle.md:20` bans **by name** because it makes `vars_files` entries silently skip; `helpers/CLAUDE.md:76` teaches a test command measured to run **0 tests and print `OK`**, the exact false-green the repo made policy against in Plan 00067; `play-nordvpn-openvpn.yml:125-134` tells the operator to run `./vault.bash set` on a variable the same play wrote seconds earlier, which `vault.bash:171` always refuses; and `extensions/CLAUDE.md` mandates `npm run lint` three times while citing as its authority the very file (`CLAUDE/QA.md:71`) that forbids it by name. The rest is omission and staleness: two docs each calling themselves *complete* both miss the same core playbook (`play-mask-intel-lpmd.yml`), the catalog misses four optional playbooks, `CLAUDE.md`'s own topic index misses two of its own current topic files, `docs/fast-file-manager.md` documents a tracker feature the play mentions **zero** times while omitting what it actually does, and three links 404. Also measured: `CLAUDE.md`'s 22 `@CLAUDE/*.md` imports contradict the doc-organisation policy stated in the same file, eagerly inlining ~108,900 bytes into every session — **left for the owner to decide**, since changing it changes what every future session sees and it was a peer review, not the owner, that raised it. Audit only; nothing fixed.
