...
- [00066-ccy-ci-runner-variant](00066-ccy-ci-runner-variant/) - `ccy` is built for one situation — a human at a workstation TTY with a desktop session — so a consuming repo (`actions-hub`, private) that needs it on a headless GitHub runner built its **own**: ~1,737 lines re-implementing image resolution, the container invocation, credential handling and the entrypoint. That is the fourth re-implementation of upstream in that estate. This plan designs the missing capabilities **at source** instead, so the consumer becomes a caller. Proven gaps: `--headless` is a *Claude-Code-invocation* mode, not a launcher mode (it changes only `claude -p` and `-it`→`-i`), so **35** `read -rp` prompts still fire and ≥10 are `while true` menu loops that **spin forever on EOF** (replicated) — unattended `ccy` can hang rather than fail; **zero** MCP support anywhere; `--network` means *attach to more*, the opposite of restriction, with no proxy/allowlist and a probe that expects the internet to work. Design decision: one launcher, layered images, orthogonal flags — a variant **image** is justified, a variant **launcher** is re-implementation #5. Design + audit only, **no code changes**; anything needing a real engine goes in `triage.bash` for a HOST run (a nested-podman attempt already failed for an unrelated userns reason and would have been mistaken for a result).
