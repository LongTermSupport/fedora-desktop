...
## 09:29 · decision · T5.5b   — restored sessions are always supervised

The owner confirmed Task 5.5b: a restored `ccy` session always comes back with the
supervisor armed, even when the original ran without it. A restored session has nobody
watching it, so arming it is the wanted default. No code change is needed.
