#!/usr/bin/env bash
#
# Plan 00073 — triage: can a stored CCY token read GET /api/oauth/usage?
#
...
# questions Plan 00073 is gated on:
...
usage() {
    cat <<'EOF'
Plan 00073 triage — probe stored CCY tokens against GET /api/oauth/usage
...
OUTPUT:
    Writes CLAUDE/Plan/00073-ccy-token-usage-limits/logs/token-usage-triage.log
...
{
    echo "================================================================================"
    echo "Plan 00073 — CCY token usage-limit triage"
    echo "================================================================================"
    ...
    for token_file in "${token_files[@]}"; do
        ...
        if [ -z "$token" ]; then
            # An empty store is the Plan 00067 failure mode — report it loudly
