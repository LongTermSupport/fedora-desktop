# Plan 00072: rclone RC auth broke every un-migrated RC client
...
Plan 00067 authenticated the rclone remote-control API: the mount units dropped
...
poll. A second, independent failure compounds it: the one client Plan 00067 *did*
fix in the repo (`ftp-camera`) was never **deployed**, because 00067's
...
- Revisiting the decision to authenticate the RC. Plan 00067's conclusion stands;
...
| F3  | Deployed `~/.local/bin/ftp-camera` is the **pre-00067** build; repo copy has the credential block     | `diff` deployed vs repo         |
...
| F6  | The mount, credential file and RC endpoint are all healthy — nothing is wrong with 00067's deployment | unit status, `vfs/stats`        |

F1 refutes the premise written into `ftp-camera` and `rclone-cache-warm`, and
into Plan 00067's own notes.
...
- [x] ✅ **Task 3.3**: Append a correction to Plan 00067's journal recording that
...
- Depends on: Plan 00067 (Complete) — this plan repairs its incomplete rollout
...
**Options considered**: (a) keep the silent fallback for pre-00067 mounts;
...
- **Plan 00067's archived `deploy.bash` keeps the loose `pgrep -af 'ftp-camera'`
...
| ------------------------------------------------------------------- | ------ | ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Deploying restarts the mount and interrupts an in-flight write-back | H      | L           | `deploy.bash` refuses while an `ftp-camera` process is running, as 00067's did                                                                                                          |
