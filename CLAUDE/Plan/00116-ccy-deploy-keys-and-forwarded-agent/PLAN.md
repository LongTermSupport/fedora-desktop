...
- [ ] ⬜ **Task 1.1**: `resolve_github_ssh_alias <url>` in `lib/ssh-handling.bash`: for a
...
- [ ] ⬜ **Task 1.2**: `discover_and_select_ssh_keys` offers the alias key: as the default row of
...
- [ ] ⬜ **Task 1.3**: `build_ssh_mounts_and_validate` probes an alias key against the alias's own
...
- [ ] ⬜ **Task 1.4**: `entrypoint.sh` appends `SSH_CONFIG_EXTRA` to `~/.ssh/config` and pins
  every host in `SSH_KNOWN_HOSTS_PINS` with the same fetched GitHub keys.

### Phase 2: Forwarded agent

- [ ] ⬜ **Task 2.1**: `--ssh-agent` flag; refused with a clear message when `SSH_AUTH_SOCK` is
  unset or `ssh-add -l` lists no key. The interactive menu offers the agent as a row whenever it
  has keys.
- [ ] ⬜ **Task 2.2**: Identity probe through the agent (`IdentityAgent=$SSH_AUTH_SOCK`,
  `IdentitiesOnly=no`), with the same 22-then-443 fallback a key gets.
- [ ] ⬜ **Task 2.3**: The container gets the socket mounted, `SSH_AUTH_SOCK` pointed at it,
...
- [ ] ⬜ **Task 2.4**: An exported `GH_TOKEN` is cross-checked against the agent's login when the
  agent gave one; the no-token error names `GH_TOKEN` as the alternative to `gh auth login`.

### Phase 3: Proof and docs

- [ ] ⬜ **Task 3.1**: `scripts/test-ccy-ssh-handling.bash` — stub `ssh` and `ssh-add` on `PATH`;
...
- [ ] ⬜ **Task 3.2**: `docs/ccy.md`: "SSH and GitHub" describes deploy-key aliases and
...
- [ ] ⬜ **Task 3.4**: Versions: `CCY_VERSION` minor bump; `LABEL claude-yolo-version` and
