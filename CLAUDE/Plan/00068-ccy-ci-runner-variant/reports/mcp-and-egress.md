# Plan 00068 — MCP injection and egress restriction

> ## Read this first — restored 2026-08-01, with three dead premises
>
> This document was deleted in the 2026-07-31 truncation (`5785c55f`) on the grounds that it
> specified retracted mechanisms, and MCP and `--egress` were then retired outright. **Both
> judgements were wrong**, and the owner caught them: the estate's own IaC provisions for both.
> `environment/dc-proxmox/group_vars/all/runner.yml:210-211` names *"the MCP GitHub server"* as
> something the runtime allowlist exists to serve, and `:251-253` keeps `registry.npmjs.org`
> allowlisted specifically so that server can be fetched at runtime. The retirement was reached
...
> three-way `--network` finding, and the proof battery.
>
> **The three dead premises**, all from the retracted "launcher is off the CI path" era:
>
> 1. **"per Phase 3, [the launcher] never runs at job time"** (Task 5.3). False — CI launches
>    `ccy` in the repo root. The launcher IS the CI path (PLAN.md Decision 3).
> 2. **"the Decision 6 CI entrypoint"** with its empty boot allowlist (Task 5.3). There is no CI
>    entrypoint. The desktop entrypoint runs, so the boot allowlist is `api.github.com` as the
>    first table in that section states.
> 3. **"`Dockerfile.ci` bakes it, per Task 3.1"** (Task 4.3). There is no `Dockerfile.ci`; the
>    project's own `.claude/ccy/Dockerfile` is the seam.
>
> **[../PLAN.md](../PLAN.md) is authoritative** where it differs from anything below.

Designed together because they share one constraint — both are **runtime** properties, so both
must be launcher/entrypoint surface or nothing.
