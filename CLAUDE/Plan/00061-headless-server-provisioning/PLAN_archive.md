...
- [x] ✅ **Task 3.2**: Added `vars: {scope: …}` to **every** play (73 plays: 31 core + 41 optional + `dev/play-collect-diagnostics.yml`, which the proposal's count missed) per `PROPOSAL.md` §1; added the exact 2-task self-guard as the first two tasks of every **gnome** play (26 gnome plays: 10 core + 16 optional; 48 general plays carry no guard; 74 scoped total). Bulk edits parallelised across 5 subagents; the byte-exact Check 4 gate verified every one.
...
- [ ] ⬜ **Task 3.8**: (On HOST) verify gating end-to-end. Automated by the plan-local **`acceptance.bash`** — a safe-by-design suite (server-path guard proof needs no `--check` because `meta: end_play` ends each gnome play before its first real task; desktop path via `-e provisioning_profile=desktop --check`; read-only detection + typo checks). Refuses to run in the CCY container. Run on the laptop host: `./CLAUDE/Plan/00061-headless-server-provisioning/acceptance.bash`.
