...
- [ ] 🔄 **Task 5.3**: **Written, not yet deployed.** `play-suspend-and-lid-policy.yml` carries the `/etc/dconf/db/gdm.d/10-power` drop-in, an unconditional `dconf update` — gating the compile on the copy having changed would leave a stale binary database unrepairable, so the play could not self-heal — and a read-back as the `gdm` account under `DCONF_PROFILE=gdm` that fails unless the greeter reads `'nothing'`. **Outstanding: run the play on the host.** A worktree cannot: playbooks never run inside one, and no syntax-check is possible there either, so both the check and the run wait on the merge to `F44`
