...
- [x] ✅ **Task 6.7**: **The no-kill gate exempts `containment.py` from `LIFECYCLE_PATTERNS` only.** It remains subject to every `SIGNAL_PATTERN`, plus `CONTAINMENT_FORBIDDEN` on top. The first attempt got this **wrong in the dangerous direction**: it skipped the file for the whole of `FORBIDDEN_PATTERNS`, which switched off all eight signal patterns for the one file permitted to run an engine command — `os.kill(pid, signal.SIGKILL)` and `["pkill", "-f", name]` both passed the gate clean, while four tracked files asserted the opposite. Caught by review, not by the gate. Seven control fixtures now pin it, including one that is specifically a containment module delivering a signal; fixture (f) could not have caught it, because `rm --force` matches two patterns that were still active
