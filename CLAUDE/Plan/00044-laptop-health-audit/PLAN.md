...
12. ~~**Blacklist `mtk_t7xx` 5G modem**~~ — 🟡 **BUSYWORK — dropped**. GNOME Settings → Mobile Network is already set to OFF (the user-facing disable). Adding a kernel-level modprobe blacklist would create a hidden second layer that fights GNOME: if the user ever flips the toggle on, nothing would happen and they'd have no idea why. The "Port AT is not opened" boot chatter is the correct cost of leaving the GNOME stack in charge. Principle: work WITH GNOME, not against it.
