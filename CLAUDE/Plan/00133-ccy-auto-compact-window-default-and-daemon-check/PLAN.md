...
- [ ] ⬜ **Task 3.4**: **Owner action, outside this container.** Review the
  ...
  resulting issue URL in `JOURNAL/`. The generated body is at
  `untracked/issue-reports/issue-report-20260917-100110.md`; the filing command is
  printed by the generator and pins `--repo`, which is not optional — without it
  `gh` files against this repository's own tracker.
