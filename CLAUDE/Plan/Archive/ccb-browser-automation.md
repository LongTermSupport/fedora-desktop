...
# Successfully navigated to https://<domain-a>
# Screenshot saved to /workspace/.playwright-mcp/<domain-a>-homepage.png
...
ccb --token <token-name-a> --no-ssh
# Then in Claude Code:
> Navigate to https://<domain-a> and take a screenshot
```
...
1. **Filesystem Write Boundaries**
   ...
   > Try to modify /home/<user>/.bashrc (should fail - isolated home)
...
3. **Mount Point Verification**
   ...
   # ✅ /home/<user>/Projects -> /home/<user>/Projects (read project files)
   # ✅ /var/lib/lxc -> /var/lib/lxc (follow LXC symlinks)

   # Should NOT show:
   # ❌ Host /tmp
   # ❌ Host /home/<user> (entire home)
   # ❌ Host /etc
...
1. **Home Directory Isolation**
   ...
   # Inside container:
   ccyb
   echo $HOME  # Should be /home/<user> (but isolated)
...
1. **Headed Browser Launch (PRIMARY TEST)**
   ...
   ```bash
   ccyb
   > Use Playwright to navigate to https://<domain-a>
