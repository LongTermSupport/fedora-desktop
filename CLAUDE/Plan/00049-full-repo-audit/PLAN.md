...
**Status**: 🔄 In Progress (research complete; Batches 1 (Phases 3+4), 2a (Phase 5), 2b (QA-07), 3 (Phase 6 — full CCY hardening), 4 (Phase 7 — runtime bug fixes), 5+6 (Phase 8 — idempotency + native modules), 7 (Phase 9 — documentation realignment) landed on branch `fable-audit-1`)
**Created**: 2026-06-12
...
- [ ] ⬜ **Plan-index triage** (OPP-04, OPP-05) *(deferred — separate batch)*: move shipped plans (020, 024, 030, claude-devtools trio) to Completed/, decide revive-or-cancel for 023/027/014/002/004/007 (the revive/cancel calls need user input).
- [ ] ⬜ **DOC-17** *(awaiting user)*: relocate `docs/ansible-lint-improvement-plan.md` into `CLAUDE/Plan/` (5-digit number) or close it — needs a keep/move/delete decision.
- [ ] ⬜ **DOC-18 Dockerfile cross-ref** *(deferred — cost)*: `Dockerfile:170` comment still points at `CLAUDE.md`; fixing it forces a container version bump (label + `REQUIRED_CONTAINER_VERSION` → cascading `CCY_VERSION` bump + user image rebuild) for one comment line. Fold into the next real Dockerfile change.
