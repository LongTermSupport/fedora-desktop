...
- **Deferred:** CCY wrapper `|| true` (`claude-yolo` ×5 + `lib/network-management.bash` ×1) → Phase 6 (needs `CCY_VERSION` bump + host rebuild + the wrapper's pre-existing shellcheck debt; rule `paths.exclude`s `files/var/local/claude-yolo/**` until then — the write-time hook still guards it). 6 Ansible `shell:`-block `|| true` still to triage (`play-AB-dnf-upgrade` ×2, `play-github-cli-multi`, `play-unifi-controller`, `play-qobuz-cli`, `play-docker-overlay2-migration`; `play-lxc-install-config`'s `ssh -T` one is load-bearing — rc 1 IS success — and stays, annotated).
