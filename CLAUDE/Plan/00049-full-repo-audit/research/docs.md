...
The doc (lines 9, 22) is built around **`play-nordvpn-cli.yml`**, which does not exist anywhere under `playbooks/`. It describes installing the official NordVPN CLI client (`nordvpn login/connect`, `nordvpnd` daemon, `nordvpn` group) plus the `NordVPN_Connect@example.com` GNOME extension.
...
- `docs/features/speech-to-text.md:759` — the link `[Containerization Guide]` with target `containerization.md` resolves to `docs/features/containerization.md` (missing); should be `../containerization.md`.
