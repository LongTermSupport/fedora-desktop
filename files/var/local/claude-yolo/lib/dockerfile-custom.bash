...
2. **Write Dockerfile** to PROMPT_EOF
    echo "$project_subdir/Dockerfile"
    cat << 'PROMPT_EOF'
...
4. **Format Example**:
   ...
   #
   # This Dockerfile extends PROMPT_EOF
    echo "$base_image"
    cat << 'PROMPT_EOF' with project-specific tools.
   # Rebuild with: PROMPT_EOF
    echo "$rebuild_cmd"
    cat << 'PROMPT_EOF'
...
```
════════════════════════════════════════════════════════════════════════════════
✓ Dockerfile created and validated: PROMPT_EOF
    echo "$project_subdir/Dockerfile"
    cat << 'PROMPT_EOF'
════════════════════════════════════════════════════════════════════════════════
...
NEXT STEPS:
  ...
  2. Rebuild the image: PROMPT_EOF
    echo "$rebuild_cmd"
    cat << 'PROMPT_EOF'
  3. Launch with custom image: PROMPT_EOF
    echo "$tool_name"
    cat << 'PROMPT_EOF'
...
MODIFY LATER:
  • PROMPT_EOF
    echo "$tool_name --custom-docker"
    cat << 'PROMPT_EOF'    (guided creation/update workflow)
  • vim PROMPT_EOF
    echo "$project_subdir/Dockerfile"
    cat << 'PROMPT_EOF'  (manual editing)
