#!/bin/bash
# Claude Code Browser Mode Container Entrypoint
# In rootless Docker, UID 0 = host user, so this is safe
# IMPORTANT: Uses ccb-specific tokens from ~/.claude-tokens/ccy/ (shared with ccy)
...
if [ -n "$SSH_KEY_PATHS" ]; then
    ...
    echo "     ccb --ssh-key ~/.ssh/github_<alias>"
    echo ""
    ...
    echo "     ccb --ssh-key ~/.ssh/id_ed25519"
    echo ""
    echo "════════════════════════════════════════════════════════════════════════════════"
    echo ""
fi
...
# Note: MCP servers are configured via --mcp-config flag passed to claude command
# See /opt/claude-browser/mcp_servers.json

# Clean up any stale Playwright MCP browser contexts from interrupted sessions
# This prevents "Browser is already in use" errors on container start
