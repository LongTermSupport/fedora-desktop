...
On networks that firewall outbound port 22, GitHub SSH (`git@github.com`) breaks.
This repo can route it over `ssh.github.com:443` instead — temporarily (the
`github-ssh-443` CLI), always-on (the `github_ssh_over_443` Ansible variable), or
automatically inside `ccy`. It also integrates with the per-account `git-<alias>`
helpers documented above.

**This has its own dedicated guide:
[GitHub SSH over Port 443](github-ssh-over-443.md).** Quickest fix on a blocked
network:

```bash
github-ssh-443 auto && eval "$(github-ssh-443 env)"
```
