...
- Signs every commit and tag with this machine's SSH signing key, which it generates
  without a passphrase so agents can sign too. `play-github-cli-multi.yml` adds a key per
  GitHub account for that account's repositories, and registers them all. ccy carries the
  project's key into its containers. See [configuration.md](configuration.md#commit-signing).
...
- **A commit-signing key per account**, used in that account's repositories and
  registered on it, with the machine's signing key registered too. See
...
3. SSH keys and signing keys are generated and uploaded per account automatically
