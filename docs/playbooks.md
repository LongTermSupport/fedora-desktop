...
**Run a play by its path, from any directory, never under `sudo` and never via a bare
`ansible-playbook`.** Every play's shebang hands it to `run.bash`, which changes to the
repo root (Ansible only finds `ansible.cfg` there), probes sudo, and adds
`--ask-become-pass` only when the box needs it. That matters because Ansible runs
`become` in its own pty, so a sudo ticket in your shell is invisible to it: on ordinary
password sudo, which is every server profile and any desktop without NOPASSWD, a bare
`ansible-playbook` run dies at its first privileged task with `sudo: a password is required`. Running under `sudo` fails differently, because root has no `ansible-playbook`.
...
./playbooks/playbook-main.yml -e provisioning_profile=server
./playbooks/playbook-main.yml -e provisioning_profile=desktop
```
...
- Moves bash history (user and root) to a private `~/.local/state/bash/history`, written at
  ...
  current directory's commands first, with up-arrow kept to the current terminal — see
...
- On the **desktop** profile, installs ABRT, sets the crash-reporting policy
  (`abrt_auto_reporting`) and installs a daily timer that removes problem records older
  than `abrt_retention_days` — see [configuration.md](configuration.md#crash-reporting-abrt)
...
- Adds a bashrc prompt offering to start `ssh-agent` on SSH sessions without one
- Signs every commit and tag with this machine's login key, `~/.ssh/id`, through the
  ssh-agent that holds it unlocked. `play-github-cli-multi.yml` signs each GitHub account's
  repositories with that account's login key, and registers them all as signing keys. A
  ccy container signs with the session's own SSH key. See
  [configuration.md](configuration.md#commit-signing).
...
- **Commit signing per account**: each account's login key signs in that account's
  repositories and is registered on it as a signing key, with `~/.ssh/id` registered too.
  See [configuration.md](configuration.md#commit-signing).
...
3. SSH keys are generated per account, and uploaded as login and signing keys, automatically
...
./playbooks/imports/optional/<category>/<playbook>.yml
```
...
- **Rootful, unlike `podfreeze`** — `/var/lib/lxc` is root-only, so even the
  read-only inventory escalates. It says so before prompting, and a refused `sudo` is
  a named failure rather than an empty machine
...
- Removes any locally built darktable (release tag `.ai`), the retired `darktable-ai` nightly, and the mock chroots and package used to build it
...
Unattended self-update for an always-on headless server that hosts ccy sessions (server
profile, Plan 00137). **Off unless `self_update_enabled: true`**; turning it off removes
