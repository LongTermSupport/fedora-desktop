...
passed over, because podman can open it; a foreign entry inside one is the one kind the check
cannot see, and podman then stops with its own error. A session with a forwarded agent (`--ssh-agent`) runs
