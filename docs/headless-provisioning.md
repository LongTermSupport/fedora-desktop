...
| `RUN_BASH_CCY_RESTORE_SESSIONS` | `1` or `0` declares `ccy_restore_sessions: true` or `false` in `localhost.yml` on every run, so ccy and cc sessions restart after a reboot ([ccy.md](ccy.md)). It is written to the file, not passed for one run, so later single-play runs keep it.                                     | No (unset: file kept) |
