...
Run any play by its path, e.g. `./playbooks/imports/play-podman.yml`, from any directory. Its shebang routes through `run.bash`, which sorts out sudo for you. Never run a play under `sudo` (root has no `ansible-playbook`) or via a bare `ansible-playbook` (on password sudo it fails with `sudo: a password is required`). Anything after the path goes to `ansible-playbook`. Details: [docs/playbooks.md](docs/playbooks.md).
